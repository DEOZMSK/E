...
export function LegalFooter({ className = "" }: { className?: string }) {
  ...
  return (
    <footer
      className={`border-t border-white/10 bg-[#120d11]/90 backdrop-blur-sm text-neutral-400 ${className}`.trim()}
    >
      <div className="mx-auto flex max-w-6xl flex-col gap-4 px-6 py-6 text-xs sm:flex-row sm:items-center sm:justify-between sm:px-8 sm:py-7 sm:text-sm">
        <div className="space-y-1 text-center sm:text-left">
          <div className="font-medium text-neutral-200">
            Елена Ксорос · фитнес-тренер и лицо проекта EFITNES
          </div>
          <div className="text-neutral-400">
            Артемий Ксорос · оператор сайта/IT-инфраструктуры, самозанятый (ИНН 500119421000)
          </div>
          ...
          <div className="text-neutral-500">© {year} EFITNES</div>
        </div>
