...
export function TrainerToolsClient({ cards }: TrainerToolsClientProps) {
  ...
  const [bpBase, setBpBase] = useState(120); const [bpAfter, setBpAfter] = useState(145); const [bpRec, setBpRec] = useState(126);
  ...
  const letunov = useMemo(() => calculateLetunov(bpBase, bpAfter, bpRec), [bpBase, bpAfter, bpRec]);
  ...
  return <section className="space-y-5">{/* cards */}
    ...
    <div className="rounded-3xl border border-white/10 bg-white/10 p-4 sm:p-6 space-y-4">
      ...
      <div className="rounded-2xl border border-white/10 bg-neutral-900/70 p-4">{activeTool==="anthropometry"&&<p>BMI {anthropometry.bmi}, WHR {anthropometry.whr}, WHtR {anthropometry.whtr}, {anthropometry.bodyType}, ориентир: {anthropometry.idealWeightKg} кг</p>}{activeTool==="calories"&&<p>BMR {calories.bmr}, TDEE {calories.tdee}, цель {calories.targetCalories} ккал / Б{calories.proteinG} Ж{calories.fatG} У{calories.carbsG}</p>}{activeTool==="caliper"&&<p>Σ складок {caliper.sumFolds} мм, жир {caliper.fatPercent}% ({caliper.fatMassKg} кг), БМТ {caliper.lbmKg} кг</p>}{activeTool==="strength"&&<p>Индекс силы: {strength.score} ({strength.level})</p>}{activeTool==="flexibility"&&<p>Наклон: {flexibility.sitAndReachCm} см, уровень: {flexibility.level}</p>}{activeTool==="functional"&&<p>PWC170: {functional.pwc170} кгм/мин, МПК: {functional.vo2max} мл/кг/мин</p>}{activeTool==="letunov"&&<p>Подъём: {letunov.rise}, восстановление: {letunov.recovery}, тип реакции: {letunov.type}</p>}{activeTool==="stress"&&<p>Балл: {stress.score}, зона: {stress.zone}</p>}{activeTool==="hypertrophy"&&<p>Тоннаж: {hypertrophy.tonnage}, интенсивность: {hypertrophy.intensity}%, индекс объёма: {hypertrophy.volumeIndex}</p>}</div>
